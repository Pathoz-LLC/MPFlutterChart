...
class CombinedData extends BarLineScatterCandleBubbleData<
  ...
  void calcMinMax1() {
    ...
    for (ChartData data in allData) {
      ...
      List<IBarLineScatterCandleBubbleDataSet<Entry>> sets = data.dataSets
          .map((e) => e as IBarLineScatterCandleBubbleDataSet<Entry>)
          .toList();
  ...
  Entry? getEntryForHighlight(Highlight highlight) {
    ...
    var dsIdx = data.getDataSetByIndex(highlight.dataSetIndex);
    if (dsIdx == null) {
      return null;
    }

    List<Entry> entries = dsIdx.getEntriesForXValue(highlight.x);
  ...
  int getDataIndex(ChartData data) {
    var x = data as BarLineScatterCandleBubbleData<
        IBarLineScatterCandleBubbleDataSet<Entry>>;
    if (x == null) {
      throw UnimplementedError('wrong data type from NS conv');
    }
    return getAllData().indexOf(x);
  }
